...
export function useSignaling(callbacks: SignalingCallbacks) {
  ...
  useEffect(() => {
    // Always connect to the same origin — works both locally and on the deployed Replit URL.
    // window.location.origin resolves to the correct server (e.g. https://xxx.janeway.replit.dev)
    // and never hardcodes localhost.
    ...
    socket.on("offer", (data) => {
      log("info", `Offer received from ${data.from}`);
